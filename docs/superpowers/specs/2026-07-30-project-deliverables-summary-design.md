...
注：`project:read` 在权限表中对 admin / engineer / production / guest 四个角色全部开放，因此不存在"无权限角色"可用于构造 403 用例，改测 404 路径。
...
- 搜索过滤（编号命中、名称命中、都不命中、大小写与空格）
- 状态过滤与「全部状态」，以及搜索与状态叠加
- 状态选项去重与顺序
- 状态标签映射（零部件类、变更类、未知值原样返回）
- 来源任务悬浮提示拼接（单任务、多任务、空数组）
- TAB 配置正确性（变更 TAB 不显示版本列、名称列表头为「标题」）

来源任务单元格的按钮与 `+N` 下拉是带交互的 JSX，不抽纯函数、不做快照测试，靠手工验证覆盖。`findTaskById` 解析失败的降级提示是 `Projects.tsx` 内两行防御式判断，同样由手工验证覆盖。
