"""
ECR (Engineering Change Request) - Pydantic Schemas
====================================================
变更管理 - ECR 模块请求/响应模型
"""

from pydantic import BaseModel, Field, ConfigDict
from typing import Optional, Literal, List, Any, Dict
from datetime import datetime
import uuid


# ============================================================
# 基础配置
# ============================================================

class BaseSchema(BaseModel):
    model_config = ConfigDict(from_attributes=True)


# ============================================================
# 辅助 Schema
# ============================================================

class ECRReviewerItem(BaseModel):
    """审批人项"""
    user_id: str
    seq: int = 0


class ECRReviewerDetail(BaseModel):
    """审批人详情（含冗余信息）"""
    seq: int
    user_id: str
    user_name: str
    role: str


class ECRReviewRecordItem(BaseModel):
    """审批记录项"""
    id: str
    reviewer_id: str
    reviewer_name: str
    decision: str
    comment: Optional[str] = None
    created_at: datetime


class ECRDocumentLinkItem(BaseModel):
    """关联图文档项"""
    document_id: str  # 现为 DocumentRevision.id（历史命名保留）
    document_code: str
    document_name: str
    document_version: str


class ECRStatusLogItem(BaseModel):
    """状态变更日志项"""
    id: str
    from_status: Optional[str] = None
    to_status: str
    operator_name: str
    comment: Optional[str] = None
    created_at: datetime


# ============================================================
# ECR 创建/编辑 Schema
# ============================================================

class ECRCreate(BaseSchema):
    """ECR 创建请求"""
    title: str = Field(..., max_length=255)
    description: Optional[str] = None
    reason: str
    priority: Literal["urgent", "high", "normal", "low"] = "normal"
    category: Optional[str] = None
    reviewers: List[ECRReviewerItem] = []
    review_mode: Literal["all", "any"] = "all"
    document_links: List[ECRDocumentLinkItem] = []


class ECREdit(BaseSchema):
    """ECR 编辑请求（所有字段可选）"""
    title: Optional[str] = Field(None, max_length=255)
    description: Optional[str] = None
    reason: Optional[str] = None
    priority: Optional[Literal["urgent", "high", "normal", "low"]] = None
    category: Optional[str] = None
    reviewers: Optional[List[ECRReviewerItem]] = None
    review_mode: Optional[Literal["all", "any"]] = None
    document_links: Optional[List[ECRDocumentLinkItem]] = None


# ============================================================
# ECR 列表 Schema
# ============================================================

class ECRListParams(BaseSchema):
    """ECR 列表查询参数"""
    page: int = Field(1, ge=1)
    page_size: int = Field(20, ge=1, le=100)
    search: Optional[str] = None
    status: Optional[str] = None
    priority: Optional[str] = None


class ECRListItem(BaseSchema):
    """ECR 列表项"""
    id: uuid.UUID
    ecr_number: str
    title: str
    status: str
    priority: str
    category: Optional[str] = None
    creator_name: str
    reviewers_count: int = 0
    approved_count: int = 0
    affected_count: int = 0
    created_at: datetime
    updated_at: datetime


# ============================================================
# ECR 详情 Schema
# ============================================================

class ECRDetail(ECRListItem):
    """ECR 详情（继承列表项 + 扩展）"""
    description: Optional[str] = None
    reason: str
    review_mode: str
    reviewers: List[ECRReviewerDetail] = []
    review_records: List[ECRReviewRecordItem] = []
    document_links: List[ECRDocumentLinkItem] = []
    affected_items: List["AffectedItemDetail"] = []
    status_logs: List[ECRStatusLogItem] = []
    reviewed_at: Optional[datetime] = None
    closed_at: Optional[datetime] = None
    eco_id: Optional[str] = None


# ============================================================
# 审批操作 Schema
# ============================================================

class ECRReviewAction(BaseSchema):
    """审批操作请求"""
    decision: Literal["approved", "rejected", "returned"]
    comment: Optional[str] = None


class ECRCloseAction(BaseSchema):
    """关闭 ECR 请求"""
    comment: Optional[str] = None


# ============================================================
# 受影响对象 Schema
# ============================================================

class AffectedItemCreate(BaseSchema):
    """添加受影响对象请求"""
    entity_type: Literal["part", "assembly", "component"]
    entity_id: str
    change_description: Optional[str] = None
    change_type: Optional[str] = None


class BomImpactNode(BaseSchema):
    """BOM 影响节点（向上/向下通用）"""
    entity_type: str
    entity_id: str
    entity_code: str
    entity_name: str
    entity_version: str
    quantity: float
    action: str = "no_change"
    target_version: Optional[str] = None
    quantity_change: Optional[Dict[str, Any]] = None
    change_description: Optional[str] = None
    # 向上链专用
    level: Optional[int] = None
    parent_entity_id: Optional[str] = None
    parent_entity_code: Optional[str] = None
    parent_target_version: Optional[str] = None
    is_change_target: bool = False
    # 向下链专用
    selected: bool = True


class AffectedItemDetail(BaseSchema):
    """受影响对象详情"""
    id: str
    entity_type: str
    entity_id: str
    entity_code: str
    entity_name: str
    entity_version: str
    change_description: Optional[str] = None
    change_type: Optional[str] = None
    bom_impact: Dict[str, Any] = {}
