...
def compare_assemblies(
    ...
    # 获取装配体基本信息
    left_assembly = db.query(PartMaster).filter(PartMaster.id == left_assembly_id).first()
    right_assembly = db.query(PartMaster).filter(PartMaster.id == right_assembly_id).first()
