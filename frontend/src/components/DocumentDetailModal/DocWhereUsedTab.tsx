...
const State = ({ s, children }: any) =>
  s.loading ? <div className="text-[var(--ui-text-tertiary)] text-sm py-2">加载中...</div>
    : s.error ? <div className="text-red-500 text-sm py-2">加载失败</div>
    : s.data.length === 0 ? <div className="text-[var(--ui-text-tertiary)] text-sm py-2">暂无引用</div>
    : children;
  ...
  return (
    <div className="space-y-2">
      <Section title="被构型项引用" count={cfg.loading ? '…' : cfg.data.length}>
        <State s={cfg}>
          <table className={tbl}><thead className="bg-[var(--ui-bg-subtle)] border-b"><tr>
      ...
      <Section title="被零部件引用" count={prt.loading ? '…' : prt.data.length}>
        <State s={prt}>
          <table className={tbl}><thead className="bg-[var(--ui-bg-subtle)] border-b"><tr>
            ...
            {prt.data.map((r: any) => (
              <tr key={r.master_id} className="hover:bg-[var(--ui-bg-hover)] cursor-pointer"
      ...
      <Section title="被 ECO 引用" count={eco.loading ? '…' : eco.data.length}>
        <State s={eco}>
          <table className={tbl}><thead className="bg-[var(--ui-bg-subtle)] border-b"><tr>
            ...
            {eco.data.map((r: any) => (
              <tr key={r.eco_id} className="hover:bg-[var(--ui-bg-hover)] cursor-pointer"
                  onClick={() => props.onOpenEco(r.eco_id)}>
                <td className="px-3 py-2 font-medium">{r.eco_number}</td>
                <td className="px-3 py-2">{r.title}</td>
                <td className="px-3 py-2 text-[var(--ui-text-secondary)]">{getStatusLabel(r.status) || r.status}</td>
              </tr>))}
          </tbody></table>
        </State>
      </Section>
      ...
      <Section title="被 ECR 引用" count={ecr.loading ? '…' : ecr.data.length}>
        <State s={ecr}>
          <table className={tbl}><thead className="bg-[var(--ui-bg-subtle)] border-b"><tr>
            ...
            {ecr.data.map((r: any) => (
              <tr key={r.ecr_id} className="hover:bg-[var(--ui-bg-hover)] cursor-pointer"
