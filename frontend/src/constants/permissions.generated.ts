// AUTO-GENERATED by tools/gen_permissions.py — DO NOT EDIT BY HAND.

export type Role = "admin" | "engineer" | "production" | "guest";
export type Permission = "admin.soft_delete:cleanup" | "admin.soft_delete:read" | "assistant:chat" | "assistant:download_artifact" | "attachments:archive_browse" | "attachments:convert_manage" | "attachments:delete" | "attachments:direct_download" | "attachments:download" | "attachments:gltf" | "attachments:list" | "attachments:preview" | "attachments:upload" | "bom:compare" | "bom:create_relation" | "bom:delete_relation" | "bom:doc_refs" | "bom:export" | "bom:trace" | "bom:tree" | "configuration.doc:manage" | "configuration.item:manage" | "configuration:checkin" | "configuration:checkout" | "configuration:create" | "configuration:delete" | "configuration:force_checkin" | "configuration:freeze" | "configuration:read" | "configuration:undocheckout" | "configuration:unfreeze" | "configuration:update" | "custom_field.def:read" | "custom_field.def:sort" | "custom_field.def:write" | "custom_field.value:read" | "custom_field.value:write" | "custom_field:reset_data" | "dashboard.folder:create" | "dashboard.folder:delete" | "dashboard.folder:rename" | "dashboard.folder:share" | "dashboard.folder:unshare" | "dashboard.item:add" | "dashboard.item:delete" | "dashboard:export_all" | "dashboard:import_all" | "dashboard:read" | "documents.attachment:delete" | "documents.attachment:download" | "documents.attachment:upload" | "documents:checkin" | "documents:checkout" | "documents:create" | "documents:delete" | "documents:force_checkin" | "documents:freeze" | "documents:read" | "documents:read_refs" | "documents:undocheckout" | "documents:unfreeze" | "documents:update" | "eco.affected:manage" | "eco:bom_trace" | "eco:cc_manage" | "eco:close" | "eco:create" | "eco:delete" | "eco:execute" | "eco:execute_all" | "eco:execute_item" | "eco:freeze" | "eco:publish" | "eco:read" | "eco:read_status_log" | "eco:restore" | "eco:revise" | "eco:submit" | "eco:update" | "eco:withdraw" | "ecr:approve" | "ecr:bom_trace" | "ecr:cc_manage" | "ecr:close" | "ecr:create" | "ecr:delete" | "ecr:read" | "ecr:read_status_log" | "ecr:submit" | "ecr:update" | "ecr:withdraw" | "inventory.doc:delete" | "inventory.doc:post" | "inventory.doc:read" | "inventory.doc:submit_withdraw_approve" | "inventory.doc:write" | "inventory.material:delete" | "inventory.material:enable_from_pdm" | "inventory.material:read" | "inventory.material:write" | "inventory.stock:read" | "inventory.warehouse:delete" | "inventory.warehouse:read" | "inventory.warehouse:write" | "logs:read" | "nav.board" | "nav.configuration" | "nav.dashboard" | "nav.documents" | "nav.ec" | "nav.help" | "nav.inventory" | "nav.parts" | "nav.projects" | "nav.settings" | "nav.users" | "notifications:manage_own" | "notifications:read" | "parts.doc:link" | "parts.doc:read" | "parts.doc:unlink" | "parts:cascade_checkin" | "parts:cascade_checkout" | "parts:cascade_undocheckout" | "parts:checkin" | "parts:checkout" | "parts:create" | "parts:delete" | "parts:export" | "parts:force_checkin" | "parts:freeze" | "parts:obsolete" | "parts:read" | "parts:release" | "parts:undocheckout" | "parts:unfreeze" | "parts:update" | "parts:upgrade" | "profile.bom:manage" | "profile:activate_archive" | "profile:approve" | "profile:cc_manage" | "profile:change_status" | "profile:create" | "profile:delete" | "profile:read" | "profile:update" | "project.member:manage" | "project.task:comment" | "project.task:create" | "project.task:delete" | "project.task:depend" | "project.task:link" | "project.task:update" | "project.task:update_status" | "project:create" | "project:delete" | "project:read" | "project:update" | "sync:read" | "user_groups:manage" | "user_groups:read" | "users:create" | "users:delete" | "users:read" | "users:read_detail" | "users:update";

export const PERMISSIONS: Record<Permission, Role[]> = {
  "admin.soft_delete:cleanup": ["admin"],
  "admin.soft_delete:read": ["admin"],
  "assistant:chat": ["admin", "engineer", "production", "guest"],
  "assistant:download_artifact": ["admin", "engineer", "production", "guest"],
  "attachments:archive_browse": ["admin", "engineer", "production"],
  "attachments:convert_manage": ["admin"],
  "attachments:delete": ["admin", "engineer"],
  "attachments:direct_download": ["admin", "engineer", "production", "guest"],
  "attachments:download": ["admin", "engineer", "production", "guest"],
  "attachments:gltf": ["admin", "engineer", "production"],
  "attachments:list": ["admin", "engineer", "production", "guest"],
  "attachments:preview": ["admin", "engineer", "production", "guest"],
  "attachments:upload": ["admin", "engineer"],
  "bom:compare": ["admin", "engineer", "production"],
  "bom:create_relation": ["admin", "engineer"],
  "bom:delete_relation": ["admin", "engineer"],
  "bom:doc_refs": ["admin", "engineer", "production"],
  "bom:export": ["admin", "engineer", "production"],
  "bom:trace": ["admin", "engineer", "production"],
  "bom:tree": ["admin", "engineer", "production"],
  "configuration.doc:manage": ["admin", "engineer"],
  "configuration.item:manage": ["admin", "engineer"],
  "configuration:checkin": ["admin", "engineer"],
  "configuration:checkout": ["admin", "engineer"],
  "configuration:create": ["admin", "engineer"],
  "configuration:delete": ["admin"],
  "configuration:force_checkin": ["admin"],
  "configuration:freeze": ["admin", "engineer"],
  "configuration:read": ["admin", "engineer", "production", "guest"],
  "configuration:undocheckout": ["admin", "engineer"],
  "configuration:unfreeze": ["admin"],
  "configuration:update": ["admin", "engineer"],
  "custom_field.def:read": ["admin", "engineer", "production", "guest"],
  "custom_field.def:sort": ["admin"],
  "custom_field.def:write": ["admin"],
  "custom_field.value:read": ["admin", "engineer", "production", "guest"],
  "custom_field.value:write": ["admin", "engineer"],
  "custom_field:reset_data": ["admin"],
  "dashboard.folder:create": ["admin", "engineer", "production", "guest"],
  "dashboard.folder:delete": ["admin", "engineer", "production", "guest"],
  "dashboard.folder:rename": ["admin", "engineer", "production", "guest"],
  "dashboard.folder:share": ["admin", "engineer", "production", "guest"],
  "dashboard.folder:unshare": ["admin", "engineer", "production", "guest"],
  "dashboard.item:add": ["admin", "engineer", "production", "guest"],
  "dashboard.item:delete": ["admin", "engineer", "production", "guest"],
  "dashboard:export_all": ["admin"],
  "dashboard:import_all": ["admin"],
  "dashboard:read": ["admin", "engineer", "production", "guest"],
  "documents.attachment:delete": ["admin", "engineer"],
  "documents.attachment:download": ["admin", "engineer", "production", "guest"],
  "documents.attachment:upload": ["admin", "engineer"],
  "documents:checkin": ["admin", "engineer"],
  "documents:checkout": ["admin", "engineer"],
  "documents:create": ["admin", "engineer"],
  "documents:delete": ["admin"],
  "documents:force_checkin": ["admin"],
  "documents:freeze": ["admin", "engineer"],
  "documents:read": ["admin", "engineer", "production", "guest"],
  "documents:read_refs": ["admin", "engineer", "production", "guest"],
  "documents:undocheckout": ["admin", "engineer"],
  "documents:unfreeze": ["admin"],
  "documents:update": ["admin", "engineer"],
  "eco.affected:manage": ["admin", "engineer"],
  "eco:bom_trace": ["admin", "engineer", "production", "guest"],
  "eco:cc_manage": ["admin", "engineer", "production", "guest"],
  "eco:close": ["admin", "engineer"],
  "eco:create": ["admin", "engineer"],
  "eco:delete": ["admin", "engineer"],
  "eco:execute": ["admin", "engineer"],
  "eco:execute_all": ["admin", "engineer"],
  "eco:execute_item": ["admin", "engineer"],
  "eco:freeze": ["admin", "engineer"],
  "eco:publish": ["admin", "engineer"],
  "eco:read": ["admin", "engineer", "production", "guest"],
  "eco:read_status_log": ["admin", "engineer", "production", "guest"],
  "eco:restore": ["admin", "engineer"],
  "eco:revise": ["admin", "engineer"],
  "eco:submit": ["admin", "engineer"],
  "eco:update": ["admin", "engineer"],
  "eco:withdraw": ["admin", "engineer"],
  "ecr:approve": ["admin", "engineer"],
  "ecr:bom_trace": ["admin", "engineer", "production", "guest"],
  "ecr:cc_manage": ["admin", "engineer", "production", "guest"],
  "ecr:close": ["admin", "engineer"],
  "ecr:create": ["admin", "engineer"],
  "ecr:delete": ["admin", "engineer"],
  "ecr:read": ["admin", "engineer", "production", "guest"],
  "ecr:read_status_log": ["admin", "engineer", "production", "guest"],
  "ecr:submit": ["admin", "engineer"],
  "ecr:update": ["admin", "engineer"],
  "ecr:withdraw": ["admin", "engineer"],
  "inventory.doc:delete": ["admin", "engineer", "production"],
  "inventory.doc:post": ["admin", "engineer", "production"],
  "inventory.doc:read": ["admin", "engineer", "production", "guest"],
  "inventory.doc:submit_withdraw_approve": ["admin", "engineer", "production"],
  "inventory.doc:write": ["admin", "engineer", "production"],
  "inventory.material:delete": ["admin"],
  "inventory.material:enable_from_pdm": ["admin", "engineer"],
  "inventory.material:read": ["admin", "engineer", "production", "guest"],
  "inventory.material:write": ["admin", "engineer"],
  "inventory.stock:read": ["admin", "engineer", "production", "guest"],
  "inventory.warehouse:delete": ["admin"],
  "inventory.warehouse:read": ["admin", "engineer", "production", "guest"],
  "inventory.warehouse:write": ["admin", "engineer"],
  "logs:read": ["admin"],
  "nav.board": ["admin", "engineer", "production", "guest"],
  "nav.configuration": ["admin", "engineer", "production", "guest"],
  "nav.dashboard": ["admin", "engineer", "production", "guest"],
  "nav.documents": ["admin", "engineer", "production", "guest"],
  "nav.ec": ["admin", "engineer", "production", "guest"],
  "nav.help": ["admin", "engineer", "production", "guest"],
  "nav.inventory": ["admin", "engineer", "production", "guest"],
  "nav.parts": ["admin", "engineer", "production", "guest"],
  "nav.projects": ["admin", "engineer", "production", "guest"],
  "nav.settings": ["admin", "engineer", "production", "guest"],
  "nav.users": ["admin", "engineer", "production"],
  "notifications:manage_own": ["admin", "engineer", "production", "guest"],
  "notifications:read": ["admin", "engineer", "production", "guest"],
  "parts.doc:link": ["admin", "engineer"],
  "parts.doc:read": ["admin", "engineer", "production", "guest"],
  "parts.doc:unlink": ["admin", "engineer"],
  "parts:cascade_checkin": ["admin", "engineer"],
  "parts:cascade_checkout": ["admin", "engineer"],
  "parts:cascade_undocheckout": ["admin", "engineer"],
  "parts:checkin": ["admin", "engineer"],
  "parts:checkout": ["admin", "engineer"],
  "parts:create": ["admin", "engineer"],
  "parts:delete": ["admin"],
  "parts:export": ["admin", "engineer", "production"],
  "parts:force_checkin": ["admin"],
  "parts:freeze": ["admin", "engineer"],
  "parts:obsolete": ["admin", "engineer"],
  "parts:read": ["admin", "engineer", "production", "guest"],
  "parts:release": ["admin", "engineer"],
  "parts:undocheckout": ["admin", "engineer"],
  "parts:unfreeze": ["admin"],
  "parts:update": ["admin", "engineer"],
  "parts:upgrade": ["admin", "engineer"],
  "profile.bom:manage": ["admin", "engineer"],
  "profile:activate_archive": ["admin", "engineer"],
  "profile:approve": ["admin", "engineer", "production"],
  "profile:cc_manage": ["admin", "engineer", "production", "guest"],
  "profile:change_status": ["admin"],
  "profile:create": ["admin", "engineer"],
  "profile:delete": ["admin"],
  "profile:read": ["admin", "engineer", "production", "guest"],
  "profile:update": ["admin", "engineer"],
  "project.member:manage": ["admin", "engineer"],
  "project.task:comment": ["admin", "engineer", "production"],
  "project.task:create": ["admin", "engineer", "production"],
  "project.task:delete": ["admin", "engineer", "production"],
  "project.task:depend": ["admin", "engineer", "production"],
  "project.task:link": ["admin", "engineer", "production"],
  "project.task:update": ["admin", "engineer", "production"],
  "project.task:update_status": ["admin", "engineer", "production"],
  "project:create": ["admin", "engineer"],
  "project:delete": ["admin", "engineer"],
  "project:read": ["admin", "engineer", "production", "guest"],
  "project:update": ["admin", "engineer"],
  "sync:read": ["admin", "engineer", "production", "guest"],
  "user_groups:manage": ["admin"],
  "user_groups:read": ["admin", "engineer", "production"],
  "users:create": ["admin"],
  "users:delete": ["admin"],
  "users:read": ["admin", "engineer", "production", "guest"],
  "users:read_detail": ["admin"],
  "users:update": ["admin"],
};
