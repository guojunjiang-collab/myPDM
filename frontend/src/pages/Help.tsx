...
export const sections: Section[] = [
    ...
    body: (
      <>
        ...
        <Steps
          items={[
            <>
               新用户在登录页点击 <K>飞书登录</K> 或其它飞书入口（如 <K>companyB</K>），完成飞书授权。
             </>,
            <>
        ...
        <H>多路飞书（多企业）</H>
        <P>系统支持同时接入多个企业的飞书应用，每个企业一套飞书凭证，互不影响：</P>
        <UL
          items={[
            <>默认仅启用主飞书入口 <K>feishu</K>；在 <K>.env</K> 设置 <K>FEISHU_PROVIDERS=feishu,companyB,companyC</K> 即可启用多路。</>,
            <>每个入口对应一套 <K>FEISHU_&lt;KEY大写&gt;_APP_ID / _APP_SECRET / _NAME</K> 环境变量，显示名由 <K>NAME</K> 决定。</>,
            <>不同企业的飞书身份（union_id）命名空间相互独立；同一人可分别在各企业飞书中对应不同系统账号，也可绑定到同一账号。</>,
            <>登录页与「飞书绑定」页会按配置自动渲染所有已启用的入口，无需改动代码。</>,
          ]}
        />
        ...
        <UL
          items={[
            <>
               每个飞书入口（如主飞书 <K>feishu</K> 与其它企业 <K>companyB</K>）的绑定<strong>相互独立</strong>，可分别绑定到相同或不同的系统账号。
             </>,
            <>
  ...
  },
  {
    ...
    body: (
      <>
        <P>系统提供 4 套界面主题，可随时切换：</P>
        <MiniTable
          head={['主题', '说明']}
          rows={[
            [<K>默认蓝</K>, <>经典天蓝配色（默认）。</>],
            [<K>森林绿</K>, <>沉稳森林绿，适合长时间浏览。</>],
            [<K>棕色</K>, <>暖调棕，柔和护眼。</>],
            [<K>深色</K>, <>黑夜模式，暗色护眼界面。</>],
          ]}
        />
        <Steps
          items={[
            <>进入 <K>系统设置 → 界面主题</K>。</>,
            <>
              点击任意主题卡片即可切换，<strong>立即生效</strong>并自动记住选择，下次登录仍是该主题。
            </>,
          ]}
        />
        <Tip>状态徽标颜色（如绿=成功、红=危险、琥珀=进行中）在各主题下保持一致，避免状态语义混淆。</Tip>
      </>
    ),
  },
  {
