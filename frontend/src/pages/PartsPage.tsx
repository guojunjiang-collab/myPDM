...
export default function PartsPage() {
  ...
  const revisionId = detailRevisionId || revision?.id;
  ...
  const loadDetail = useCallback(async () => {
    if (!detailMasterId) return;
    setDetailLoading(true);
    try {
      const m = await partsApi.get(detailMasterId);
      setMaster(m);
      const revId = detailRevisionId || (m.latest_revision?.id);
      if (revId) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* ==================== 详情模态框 ==================== */}
      <Modal open={!!detailMasterId} title="零部件详情" onClose={closeDetail} width="full">
                          ...
                          <tr key={v.id} className={`hover:bg-gray-50 ${v.id === revision?.id ? 'bg-blue-50' : ''}`}>
                            ...
                            <td className="px-4 py-3">
                              {v.id === revision?.id ? (
                                ...
                                <button onClick={() => setDetailRevisionId(v.id)} className="text-primary-600 hover:text-primary-800 hover:underline text-xs">切换</button>
                              )}
                            </td>
                          </tr>
                        ))}
