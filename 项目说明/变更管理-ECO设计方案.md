...
4. **变更执行**: 升版、数量变更、删除 BOM 关系、新增零件——所有操作落地到 parts/assemblies/bom_items 表
5. **多人会签审批**: ECO 拥有独立的审批流程（会签/或签），审批通过后方可执行
6. **逐项或批量执行**: 支持逐项手动执行和一键批量执行
7. **知会**: 任何用户可将 ECO 知会其他用户，知会用户可查看该 ECO
8. **权限**: 每用户只看与自己相关的 ECO（创建人/审批人/执行人/知会人），管理员看全部
...
| 项目 | 优先级 | 说明 |
|------|--------|------|
| 无 ECR 来源时 BOM 分析 | 🟡 中 | 需要先关联 ECR |
| 独立 ECO 的 BOM 树编辑器 | 🟡 中 | 需求已明确，未实施 |
| 向上溯源链中同一实体多卡片编辑冲突提示 | 🟢 低 | 目前各自独立保存，无冲突检测 |

---

*文档版本: v1.6 | 2026-05-25*
